...
class EmployeeFilter(filterset.FilterSet):
    ...
    def filter_name_oe_department(self, queryset, name, value):
        return queryset.select_related('department').filter(Q(name__icontains=value) | Q(department__name__icontains=value))

    class Meta:
        model = models.Employee
        field = ['statr_slary', 'end_salary']
